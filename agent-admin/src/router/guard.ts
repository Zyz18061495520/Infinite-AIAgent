...
import { useAppStore, useRouteStore, useTabStore, useWorkspaceStore } from '@/store'
...
export function setupRouterGuard(router: Router) {
  ...
  router.beforeEach(async (to, from, next) => {
    ...
    // 只有在用户已登录的情况下才获取workspaces
    if (isLogin && to.meta.requiresWorkspace !== false && (!workspaceStore.workspaceId || workspaceStore.workspaces.length === 0)) {
