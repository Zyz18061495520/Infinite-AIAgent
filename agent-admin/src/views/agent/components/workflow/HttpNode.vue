...
function handleNodeClick() {
  if (props.id) {
    const event = new CustomEvent('node-click', { detail: { nodeId: props.id, nodeType: 'http' } })
...
<template>
  <div class="http-node" @click="handleNodeClick">
    <!-- 顶部渐变背景，与 LLM 节点一致 -->
    <div class="top-gradient-background" />
    <div class="node-header">
      <div class="icon-circle http">
        <svg viewBox="0 0 24 24" width="12" height="12" fill="currentColor">
          <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2z" />
          <path d="M8 12l2 2 4-4" />
        </svg>
      </div>
      <span class="node-title">{{ nodeData.name || 'HTTP' }}</span>
    </div>
    ...
    <div class="node-body">
      <div class="section">
        ...
        <div class="tags-container">
          ...
          <span v-else class="placeholder">未配置HTTP</span>
        </div>
      </div>
      <div class="section">
        <div class="section-label">
...
.icon-circle.http { background: #10b981; }
...
.http-node:hover .handle-input,
.http-node:hover .handle-output {
  transform: scale(1.3);
  background: #8b5cf6;
}
