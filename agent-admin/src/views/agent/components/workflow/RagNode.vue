...
function handleNodeClick() {
  if (props.id) {
    const event = new CustomEvent('node-click', { detail: { nodeId: props.id, nodeType: 'rag' } })
...
<template>
  <div class="rag-node" @click="handleNodeClick">
    <div class="top-gradient-background" />
    <div class="node-header">
      <div class="icon-circle rag">
        <svg viewBox="0 0 24 24" width="12" height="12" fill="currentColor">
          <path d="M20 2H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h4l4 4 4-4h4c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2z" />
        </svg>
      </div>
      <span class="node-title">{{ nodeData.name || 'RAG' }}</span>
    </div>
    ...
    <div class="node-body">
      <div class="section">
        ...
        <div class="tags-container">
          ...
          <span v-else class="placeholder">未配置RAG</span>
        </div>
      </div>

      <div class="section">
        <div class="section-label"><!-- 不展示输入 --></div>
        <div class="tags-container" />
      </div>
...
.icon-circle.rag { background: #8b5cf6; }
...
.rag-node:hover .handle-input,
.rag-node:hover .handle-output {
  transform: scale(1.3);
  background: #8b5cf6;
}
